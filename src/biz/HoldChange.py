...
dis_addr_sql = 'SELECT name_eg, address, id  %s and id in ' \
               '(select max(id) %s group by name_eg,address)' % (wc, wc)
# max_id_sql = "SELECT MAX(id) AS id FROM symbol_position WHERE name_eg='%s' and address = '%s' "
...
def exec_hold_exchange():
    ...
    try:
        ...
        for r in rs:
            ...
            addr = r[1]
            maxId = r[2]
            # cursor.execute(max_id_sql % (coin_name, addr))
